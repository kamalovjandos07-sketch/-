...
selected_antibiotic = st.radio(
    "Выбери антибиотик (если есть):",
    options=["Нет"] + antibiotics_list,
    horizontal=True
)
...
selected_drugs = st.multiselect(
    "Выбери лекарства (если есть):",
    options=other_drugs_list
)
...
effects = {
    if selected_antibiotic != "Нет":
    for b in bacteria_concentrations:
        bacteria_concentrations[b] *= 0.6  # -40% после курса антибиотика
       if "Пробиотики" in selected_drugs:
    for b in bacteria_concentrations:
        bacteria_concentrations[b] *= 1.2  # +20% рост полезных бактерий

if "Антациды" in selected_drugs:
    if "E.coli" in bacteria_concentrations:
        bacteria_concentrations["E.coli"] *= 1.1  # +10% рост из-за pH

if "Гормоны" in selected_drugs:
    if "Bacteroides" in bacteria_concentrations:
        bacteria_concentrations["Bacteroides"] *= 0.9  # -10% влияние гормонов
