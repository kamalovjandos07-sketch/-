...
def analyze(updated_dict):
    ...
    for k in baseline:
        ...
        if val <= base * 0.2:
            ...
            msgs.append(f"⬆️ Умеренное увеличение {k} (1.5–5× базового)")
    ...
    if lacto < baseline["Lactobacillus spp."] * 0.5 and bifi < baseline["Bifidobacterium spp."] * 0.5:
        conclusions.append("⚠️ Патерн: снижение основных симбионтов — риск дисбактериоза")
    if proteo > baseline["Proteobacteria (проч.)"] * 2.0:
        conclusions.append("⚠️ Увеличение Proteobacteria — маркер воспаления/дисбиоза")
